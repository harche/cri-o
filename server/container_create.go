...
func (s *Server) CreateContainer(ctx context.Context, req *types.CreateContainerRequest) (res *types.CreateContainerResponse, retErr error) {
	...
	// s.Config().ContainerEventsChan <- types.ContainerEventResponse{ContainerId: newContainer.ID(), ContainerEventType: types.ContainerEventType_CONTAINER_CREATED_EVENT, PodSandboxMetadata: s.GetSandbox(newContainer.CRIContainer().PodSandboxId).Metadata()}
